  ...
  fluidPage(
    ...
    tabItems(
      ...
      tabItem(tabName = "home",
              ...
              br(),
              fluidRow(h4("This word cloud represents the distribution of majors of participants at DataFest since its inception.")),
...
server <- function(input, output, session) {
  ...
  output$wordcloud <- renderPlot({
    ...
    ggplot(words, aes(label = word, color = word, size = size)) +
      geom_text_wordcloud(max_steps = 1, grid_margin = 0.5, eccentricity = 0.6) +
  ...
  output$wordcloud_host <- renderPlot({
    majors <- filter(updated_datafest,host == input$college, year <= input$uni_year)
    majors <- majors$major_dist
    majors <- unlist(strsplit(majors, "[;]|[,]| or "))
    majors <- gsub('[[:punct:]]+' , '' , majors)
    majors <- gsub('[[:digit:]]+', '', majors)
    majors <- str_to_title(str_trim(majors))
    majors <- majors[majors != 'Ms']
    majors <- gsub('And', 'and', majors)
    majors <- gsub('Math', 'Mathematics', majors)
    majors <- gsub('Mathematicsematics', 'Mathematics', majors)
    majors <- gsub(' Ms', '', majors)
    majors <- gsub('Bmi', 'BMI', majors)
    majors <- gsub(' Note Some Were Double Majors', '', majors)
    majors <- gsub('All Participants Are Business Majors', 'Business', majors)
    ...
    set.seed(9)
    ggplot(word, aes(label = word, color = word, size = 60, angle = angle)) +
      geom_text_wordcloud(max_steps = 1, grid_margin = 0.5, eccentricity = 0.6) +
